...
object EncryptUtils {
    ...
    fun encryptMD5ToString(data: String, salt: String?): String {
        return if (salt == null) {
            bytes2HexString(encryptMD5(data.toByteArray()))
        } else {
            bytes2HexString(encryptMD5((data + salt).toByteArray()))
        }
    }
    ...
    private fun hexString2Bytes(hexString: String): ByteArray? {
        var tmpHexString = hexString
        if (isSpace(tmpHexString)) return null
        var len = tmpHexString.length
        if (len % 2 != 0) {
            tmpHexString = "0$tmpHexString"
            len += 1
        }
        val hexBytes = tmpHexString.toUpperCase().toCharArray()
